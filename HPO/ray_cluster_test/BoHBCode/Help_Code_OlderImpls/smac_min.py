...
def train(config: Configuration, seed: int = 0) -> float:
    # list process id and configuration
    classifier = SVC(C=config["C"], random_state=seed)
    print(f"Process id: {os.getpid()}, Config: {config}")
...
if __name__ == "__main__":
    ...
    id=uuid.uuid4()
    # scenario = Scenario(configspace, deterministic=True, n_trials=10, output_directory=PosixPath('smac3_output_custom'), 
    #                     n_workers=5 )
