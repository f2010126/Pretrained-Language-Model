...
def predict_multi_masked_sent(text, top_k=5):
    # Tokenize input
    text = "[CLS] %s [SEP]"%text
    ...
    for i, masked_index in enumerate(masked_indices):
        ...
        print("Predictions for [MASK] %d:"%i, predicted_tokens, top_k_weights.tolist())
...
predict_multi_masked_sent("My [MASK] is [MASK] cute.", top_k=5)
'''
