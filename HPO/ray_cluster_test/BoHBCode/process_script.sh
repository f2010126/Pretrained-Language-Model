...
if [ $SLURM_ARRAY_TASK_ID -eq 0 ];
   then python3 node_worker.py --run_id $SLURM_JOB_NAME --nic_name eth0 --shared_directory ./BOHB_Results/$SLURM_JOB_NAME --task_id $SLURM_ARRAY_TASK_ID
else
    python3 node_worker.py --run_id $SLURM_JOB_NAME --nic_name eth0  --shared_directory ./BOHB_Results/$SLURM_JOB_NAME --task_id $SLURM_ARRAY_TASK_ID --worker
fi
